...
void man (char *name)
{
    ...
    {
        printf ("Description : Used to displayy information of file\n");
...
void DisplayHelp()
{
  ...
  printf("stat : To Display information of all using name\n");
...
int CreateFile(char * name,int permission)
{
    ...
    if(SUPERBLOCKobj.FreeInode == 0)
     ...
     if(Get_Inode(name) != NULL)
      ...
      UFDTArr[i].ptrfiletable-> ptrinode->Permissin = permission;
...
int ReadFile(int fd, char *arr,int isize)
{
    ...
    if(UFDTArr[fd].ptrfiletable->ptrinode->Permissin != READ && UFDTArr[fd].ptrfiletable->ptrinode->Permissin != READ + WRITE)  return -2;
...
int WriteFile(int fd, char *arr,int isize)
{
   if(((UFDTArr[fd].ptrfiletable->mode ) !=WRITE) && ((UFDTArr[fd].ptrfiletable->mode )!=READ + WRITE))   return -1;
    ...
    if(((UFDTArr[fd].ptrfiletable->ptrinode->Permissin ) != WRITE ) && ((UFDTArr[fd].ptrfiletable->ptrinode->Permissin )!= READ + WRITE))  return -1;
...
int LseekFile(int fd, int size ,int from)
{
 ...
 else if (UFDTArr[fd].ptrfiletable->mode == WRITE)
 {
    if(from == CURRENT)
    {
       if(((UFDTArr[fd].ptrfiletable->writeoffset)+ size) > MAXFILESIZE)  return -1;   return -1;
...
int  fstat_file(int fd)
{
    ...
    else if(temp->Permissin == 2)
      printf("File Permission : Write\n");
    else if(temp->Permissin == 3)
    ...
    while(1)
    {
        ...
        count = sscanf(str,"%s %s %s %s",command[0],command[1],command[2],command[3]);
          ...
          else if (count == 2)
          {
            ...
            else if (strcmp(command[0],"teuncate") == 0)
            {
